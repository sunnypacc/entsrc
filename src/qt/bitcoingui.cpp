    ...
    rpcConsole(0)
{
    ...
    resize(850, 550);
    setWindowTitle(tr("ENTCOIN") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
{
    QSettings setting("ENTCOIN.conf", QSettings::IniFormat);
...
void BitcoinGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About ENTCOIN"), this);
    aboutAction->setToolTip(tr("Show information about ENTCOIN"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for ENTCOIN"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("ENTCOIN client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("ENTCOIN client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to ENTCOIN network", "", count));
}
...
void BitcoinGUI::viewBlockWebPage()
{
    ...
    //{
        if (!QDesktopServices::openUrl(QUrl("http://211.149.224.35:8099")))
        {
            QMessageBox::warning(this, tr("Error"), tr("Open Brower False."));
        }
    //}
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid ENTCOIN address or malformed URI parameters."));
    }
